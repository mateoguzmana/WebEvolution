						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
                                                ...
                                                $queryUSERS			="SELECT* FROM Facturado where Muestra = 0 Order by Id DESC";
												...
												$queryRS 			="SELECT * FROM Facturadomov WHERE Idfact='$IdUSERS'";
												$resultRS 			=mysql_query($queryRS,$id);
												while ($rowRS 		=mysql_fetch_array($resultRS)) {
												
												$Idobra 			=$rowRS["Idobra"];		
												}
												$queryPT			="SELECT * FROM Proyectos WHERE Id='$Idobra'";
												...
												<tr class="gradeA">
													...
													<td class="text-right">
														...
														{
														?>                                              
													<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR" style="width:100px;"  onClick="location.href='facturadas-2.php?Id=<?=$IdUSERS?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														elseif($Estado == 4 || $Estado==3)
														{
														?>                                              
													<input name="Procesar" type="button" class="btn btn-primary" value="VER" style="width:100px;"  onClick="location.href='facturadas-2.php?Id=<?=$IdUSERS?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														elseif($Estado == 1)
														{
														?>
													<input name="Procesar" type="button" class="btn btn-warning" value="FINALIZADA"  style="width:100px;" onClick="location.href='facturadas-2.php?Id=<?=$IdUSERS?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														elseif($Estado == 2)
														{
														?>
													<input name="Procesar" type="button" class="btn btn-success" value="ANULADA"  style="width:100px;" onClick="location.href='facturadas-2.php?Id=<?=$IdUSERS?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														?> 
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <?for ($i=0; $i < $x; $i++) { ?>
																	<?}?>
                                                             	<form id="Anulacion" onsubmit="return valida<?=$i?>(this)" action='eli-facturada.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>' method="post">
